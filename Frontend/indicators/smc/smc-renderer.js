...
(function(){
  ...
  function structureEvents(events){
    // Pine's structHistoryNbr keeps the latest ten accepted breaks exactly as
    // generated. Do not collapse continuation BOS events: doing so leaves old
    // CHoCH lines on screen and makes the browser overlay diverge visually.
  ...
  class SmcRenderer{
    ...
    render(structure){this.lastStructure=structure||null;const viewport=this.captureViewport();this.clear();if(!this.enabled||!this.chart||!this.candleSeries||!structure){this.restoreViewport(viewport);return;}const settings=this.cfg();structureEvents(structure.events).forEach(e=>this.addBreak(e,settings));this.addCurrentStructure(structure,settings);this.addFibs(structure,settings);this.restoreViewport(viewport);window.requestAnimationFrame(()=>this.restoreViewport(viewport));}
  }
