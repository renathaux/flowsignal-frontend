...
assert.match(account, /\/auth\/\$\{signup\?'signup':'login'\}/);
...
assert.match(auth, /logicalPath=`\/user\$\{logicalPath\}`/, 'authenticated customers use user Deriv namespace');
...
assert.match(loader, /user-auth\.js\?v=4/);
...
assert.doesNotMatch(callback, /\/user\/deriv\/oauth\/state/, 'OAuth state is registered before leaving FlowSignal, never on callback');
...
assert.match(binary, /Authoritative V5 Signal/i);
assert.match(binary, /Connect Deriv/);
assert.match(binary, /\/deriv\/oauth\/state/, 'OAuth state is bound before redirecting to Deriv');
assert.match(profile, /\/auth\/change-password/);
assert.match(profile, /Other customer sessions were signed out/);
assert.match(auth, /account\.html\?expired=1/, 'expired customer sessions get a clear login message');
