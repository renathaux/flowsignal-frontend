(function(){
  ...
  function applyUser(user){sessionUser=user||null; if(!user) return; localStorage.setItem('flowsignal_role',user.role||'user'); sessionStorage.setItem('flowsignal_tab_role',user.role||'user'); document.body.dataset.userRole=user.role||'user'; hideAuth(); document.dispatchEvent(new CustomEvent('flowsignal:authenticated',{detail:{user}}));}
  ...
  async function logout(){try{await window.fetch(`${BACKEND}/auth/logout`,{method:'POST'});}catch(_error){} sessionUser=null; csrfToken=''; sessionStorage.removeItem(CSRF_KEY); localStorage.removeItem('flowsignal_deriv_connection_id'); localStorage.removeItem('flowsignal_deriv_account_id'); if(!legacyOwner()) showAuth();}
